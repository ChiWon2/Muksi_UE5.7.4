...
void UBattleGridNavigationComponent::CacheGridManager()
{
	...
	if (!GridManager)
	{
		UE_LOG(
			...
			Error,
			TEXT(
				"[BattleGridNavigationComponent] "
				"Owner is not ABattleGridManager. Owner=%s"
			),
...
bool UBattleGridNavigationComponent::IsCellAvailable(
...
) const
{
	...
	if (
		IgnoredActor
		&& Cell->OccupyingActor == IgnoredActor
		)
	{
		return true;
	}
...
float UBattleGridNavigationComponent::GetCellMovementCost(
	const FIntPoint& Coord
) const
{
	if (!GridManager)
...
float UBattleGridNavigationComponent::CalculateHexDistance(
	const FIntPoint& A,
	const FIntPoint& B
) const
{
	...
	const FCubeCoord CubeA =
		GridManager->OddQToCube(A);

	const FCubeCoord CubeB =
		GridManager->OddQToCube(B);

	const int32 Distance =
		FMath::Max3(
			FMath::Abs(CubeA.X - CubeB.X),
			FMath::Abs(CubeA.Y - CubeB.Y),
			FMath::Abs(CubeA.Z - CubeB.Z)
		);

	return static_cast<float>(Distance);
}

bool UBattleGridNavigationComponent::FindGroundPath(
...
) const
{
	...
	if (!GridManager)
	{
		UE_LOG(
			...
			TEXT(
				"[BattleGridNavigationComponent] "
				"FindGroundPath failed: GridManager is null."
			)
		);
	...
	if (!IsCellAvailable(DestinationCoord, MovingActor))
	{
		UE_LOG(
			...
			TEXT(
				"[BattleGridNavigationComponent] "
				"Destination is not available: (%d, %d)"
			),
			DestinationCoord.X,
			DestinationCoord.Y
		);
	...
	while (!OpenSet.IsEmpty())
	{
		...
		const float CurrentGScore =
			GScore.Contains(CurrentCoord)
			? GScore[CurrentCoord]
			: TNumericLimits<float>::Max();

		const TArray<FIntPoint> Neighbors =
			GridManager->GetHexNeighbors(CurrentCoord);
		...
		for (const FIntPoint& NeighborCoord : Neighbors)
		{
			...
			const float MovementCost =
				GetCellMovementCost(NeighborCoord);
			...
			const float TentativeGScore =
				CurrentGScore + MovementCost;

			const float ExistingGScore =
				GScore.Contains(NeighborCoord)
				? GScore[NeighborCoord]
				: TNumericLimits<float>::Max();
...
bool UBattleGridNavigationComponent::FindLowestScoreCoord(
...
bool UBattleGridNavigationComponent::ReconstructPath(
	const FIntPoint& StartCoord,
	const FIntPoint& DestinationCoord,
...
) const
{
	...
	while (CurrentCoord != StartCoord)
	{
		...
		const FIntPoint* PreviousCoord =
			CameFrom.Find(CurrentCoord);
		...
		if (!PreviousCoord)
		{
			UE_LOG(
				...
				TEXT(
					"[BattleGridNavigationComponent] "
					"Failed to reconstruct path at (%d, %d)"
				),
				CurrentCoord.X,
				CurrentCoord.Y
			);
...
bool UBattleGridNavigationComponent::ConvertGridPathToWorldPath(
...
) const
{
	...
	if (!GridManager)
	{
		UE_LOG(
			...
			TEXT(
				"[BattleGridNavigationComponent] "
				"ConvertGridPathToWorldPath failed: "
				"GridManager is null."
			)
		);
	...
	for (const FIntPoint& GridCoord : GridPath)
	{
		...
		if (!GetGridWorldLocation(GridCoord, WorldLocation))
		{
			UE_LOG(
				...
				TEXT(
					"[BattleGridNavigationComponent] "
					"Failed to convert Grid Coord to World Location. "
					"Coord=(%d, %d)"
				),
				GridCoord.X,
				GridCoord.Y
...
bool UBattleGridNavigationComponent::GetGridWorldLocation(
	const FIntPoint& Coord,
...
) const
{
	...
	const FTransform TargetTransform =
		GridManager->GetTransformToPosition(Coord);
