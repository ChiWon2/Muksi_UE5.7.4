...
#include "Widgets/Components/MuksiCommonListView.h"
...
void UWidget_OptionScreen::NativeOnDeactivated()
{
	...
	UMuksiGameUserSettings::Get()->ApplySettings(true);
}
...
void UWidget_OptionScreen::OnResetBoundActionTriggered()
{
	...
	const FString SelectedTabButtonName = CastChecked<UMuksiCommonButtonBase>(SelectedTabButton)->GetButtonDisplayText().ToString();
	
	UMuksiUISubsystem::Get(this)->PushConfirmScreenToModalStackAynsc(
		EConfirmScreenType::YesNo,
		FText::FromString(TEXT("Reset")),
		FText::FromString(TEXT("Are you sure you want to reset to rest all the setting under the ") + SelectedTabButtonName + TEXT(" tab.")),
		[this](EConfirmScreenButtonType ClickedButtonType)
		{
			...
			for (UListDataObject_Base* DataToReset : ResettableDataArray)
			{
				...
				}else
				{
					...
					Debug::Print(DataToReset->GetDataDisplayName().ToString() + TEXT(" failed reset"));
				}
			}
...
void UWidget_OptionScreen::OnListViewItemHovered(UObject* InHoveredItem, bool bWasHovered)
{
	...
	if (bWasHovered)
	{
		DetailsView_ListEntryInfo->UpdateDetailsViewInfo(
			...
			TryGetEntryWidgetClassName(HoveredEntryWidget)
		);
	}else
	{
...
void UWidget_OptionScreen::NativeOnActivated()
{
	...
	for (UListDataObject_Collection* TabCollection : GetOrCreateDataRegistry()->GetRegisterOptionsTabCollections())
	{
		if (!TabCollection)
		{
			continue;
...
void UWidget_OptionScreen::OnResetBoundActionTriggered()
{
	Debug::Print(TEXT("Reset button clicked"));
}
