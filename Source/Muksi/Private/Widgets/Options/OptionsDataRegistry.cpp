...
#define MAKE_OPTIONS_DATA_CONTROL(SetterOrGetterFuncName) \
	MakeShared<FOptionsDataInteractionHelper>(GET_FUNCTION_NAME_STRING_CHECKED(UMuksiGameUserSettings, SetterOrGetterFuncName))
...
{
	if (!InParentData || !InParentData->HasChildListData())
	{
		return;
	}
	for (UListDataObject_Base* SubChildListData : InParentData->GetChildListData())
	{
		...
		if (SubChildListData->HasChildListData())
		{
...
void UOptionsDataRegistry::InitGameplayCollectionTab()
{
	...
	//Game Difficulty
	{
		...
		GameDifficulty->SetDescriptionRichText(FText::FromString(TEXT("Adjusts the difficulty of the game experience. \n\n<Bold>Easy: </> Focuses on the story experience. Provides the most relaxing combat. \n\n<Bold>Nomal:</>Offers slightrly harder combat experience. \n\n<Bold>Hard: </>Offers a much more challenging combat experience.\n\n<Bold>Very Hard:</> Provides the most challenging combat experiece. Not recommended for first play through. ")));
	...
	//Test Item
	{
		...
		TestItem->SetSoftDescriptionImage(UMuksiFunctionLibrary::GetOptionsSoftImageByTag(MuksiGameplayTag::Muksi_Image_TestImage));
...
void UOptionsDataRegistry::InitAudioCollectionTab()
{
	...
	//Volume Category
	{
		...
		//VolumeCategoryCollection->AddChildListData();
		...
		//Language Settings
		{
			UListDataObject_String* LanguageSettings = NewObject<UListDataObject_String>();
			LanguageSettings->SetDataID(FName("Language"));
			LanguageSettings->SetDataDisplayName(FText::FromString(TEXT("Language")));
			LanguageSettings->SetDescriptionRichText(FText::FromString(TEXT("This is description for Language")));
			LanguageSettings->AddDynamicOption(TEXT("en"), FText::FromString(TEXT("English")));
			LanguageSettings->AddDynamicOption(TEXT("ko"), FText::FromString(TEXT("Korean")));
			LanguageSettings->SetDefaultValueFromString(TEXT("en"));
			
			//TODO FIND FUNCTION Settings Language
			LanguageSettings->SetDataDynamicGetter(MAKE_OPTIONS_DATA_CONTROL(GetCurrentGameLanguage));
			LanguageSettings->SetDataDynamicSetter(MAKE_OPTIONS_DATA_CONTROL(SetCurrentGameLanguage));
			...
			LanguageSettings->SetShouldApplySettingsImmediately(true);
			AudioTabCollection->AddChildListData(LanguageSettings);
...
void UOptionsDataRegistry::InitVideoCollectionTab()
{
	...
	//Display Category
	{
		...
		//Screen Resolution
		{
			...
			WindowModeEditCondition.SetDisabledRichReason(TEXT("\n\n<Disabled>Screen Resolution is not adjustable when the 'Window Mode' is set Borderless Window. The value must match with the maximum allowed resolution.</>"));
	...
	//Graphics Category
	{
		...
		//AntiAliasing Quality
		{
			...
			AntiAliasingQuality->SetDescriptionRichText(FText::FromString(TEXT("This is description for AntiAliasing Quality")));
		...
		//Visual Effects Quality
		{
			...
			VisualEffectQuality->SetDescriptionRichText(FText::FromString(TEXT("This is description for Visual Effect Quality")));
		...
		//Post Processing Quality
		{
			...
			PostProcessingQuality->SetDescriptionRichText(FText::FromString(TEXT("This is description for PostProcessing Quality")));
