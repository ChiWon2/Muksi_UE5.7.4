...
bool UListDataObject_String::CanSetToForcedStringValue(const FString& InForcedValue) const
{
	...
	//return Super::CanSetToForcedStringValue(InForcedValue);
...
void UListDataObject_StringBool::OverrideTrueDisplayText(const FText& InNewTruDisplayText)
{
	if (!AvailableOptionsStringArray.Contains(TrueString))
	{
		AddDynamicOption(TrueString, InNewTruDisplayText);
	}
}
